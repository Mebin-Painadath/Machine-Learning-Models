...
def second():
    df=pd.read_csv(globals()['base_path']+'\\DecisionTreeClassifier\\MissingAttribute(Petal_Width).txt')
...
def forth():
    df=pd.read_csv(globals()['base_path']+'\\DecisionTreeClassifier\\MissingAttribute(Petal_Length).txt')
    globals()['x_test']=df.iloc[:,:4]
    globals()['y_test']=df.iloc[:,4:5]

def fifth():
    df=pd.read_csv(globals()['base_path']+'\\DecisionTreeClassifier\\MissingAttribute(Sepal_Width).txt')
    globals()['x_test']=df.iloc[:,:4]
    globals()['y_test']=df.iloc[:,4:5]
...
while(1):
    ...
    print("Enter 6 to get the graph",end="")
    try:
